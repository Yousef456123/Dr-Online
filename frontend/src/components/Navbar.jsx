          ...
          {user ? (
            <>
              <button
                ...
                className="rounded-full bg-white px-4 py-2 text-sm font-semibold text-primary-600 shadow-glass hover:bg-accent hover:text-midnight"
              >
                Logout
              </button>
              <Link
                to="/profile"
                className="h-10 w-10 overflow-hidden rounded-full border border-white/40 hover:border-white transition cursor-pointer"
              >
                {user.profileImage ? (
                  ...
                  <img 
                    ...
                    onError={(e) => {
                      ...
                      e.target.parentElement.innerHTML = `<div class="flex h-full w-full items-center justify-center bg-white text-primary-600 font-semibold">${user.fullName?.charAt(0) || 'U'}</div>`;
                    }}
                  />
                ) : (
                  <div className="flex h-full w-full items-center justify-center bg-white text-primary-600 font-semibold">{user.fullName?.charAt(0) || 'U'}</div>
                )}
              </Link>
            </>
          ) : (
          ...
          <div className="mt-4 flex flex-col gap-2">
            ...
            {user ? (
              ...
              <>
                <Link
                  to="/login"
                  className="rounded-full border border-white/40 px-5 py-2 text-center text-sm font-semibold text-white hover:bg-white/10"
                  onClick={() => setOpen(false)}
                >
                  Log In
                </Link>
